...
This document describes the major changes in flashrom version v1.7.0.
The work was done during 7 months since previous release in July 2025.
We have 30 authors and 90+ commits, thank you everyone!

Download
========

flashrom v1.7 can be downloaded in the following ways:
...
(signature https://download.flashrom.org/releases/flashrom-v1.7.0.tar.xz.asc)
...
* Using ``--progress`` option together with ``--fmap*`` options was overflooding the terminal with
  redundant messages.

  https://ticket.coreboot.org/issues/616

Added support
=============

* AT45DB641E (with added EDI detection)
...
* Serial: Fix Windows compilation error (missing header)
* doc: Update Windows building instructions
* flashchips: spansion: Add missing FEATURE_WRSR_EXT2
* platform/udelay: Document file and functions
...
* tests/probe_spi: Add more detailed tests for probing functions (probe_spi_rdid, probe_spi_res1)
